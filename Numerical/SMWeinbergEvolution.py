"""Numerical evolution of the final SM + Weinberg EFT.
...
The physical conversion C5 -> m_nu lives under ``physics.NeutrinoMass`` and is
re-exported here under the historical ``neutrino_mass_matrix`` name.
...
class SMWeinbergInitialConditions:
    gY: float
    g2: float
    g3: float
    lambdaH: float
    ...
    def validated(self) -> "SMWeinbergInitialConditions":
...
@dataclass(frozen=True)
class SMWeinbergEvolutionResult:
    mu_initial: float
    mu_final: float
    gY: float
    g2: float
...
def _pack(state: SMWeinbergInitialConditions) -> np.ndarray:
...
def evolve_sm_weinberg(
    ...
    *,
    ...
    return SMWeinbergEvolutionResult(
...
# Historical type/function aliases retained during migration.
SMInitialConditions = SMWeinbergInitialConditions
NumericalRGEResult = SMWeinbergEvolutionResult
evolve_weinberg = evolve_sm_weinberg
