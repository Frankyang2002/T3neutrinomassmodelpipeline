"""Numerical final-SM+Weinberg stage for a matched C5 coefficient.
...
substitution and integration live here under ``Numerical/``. Physical C5 -> m_nu
conversion is delegated to ``physics.NeutrinoMass``.
"""
...
def evaluate_symbolic_c5(
    ...
    for r, mass_value in enumerate(heavy_masses):
...
def run_sm_weinberg_numerical_stage(
    c5_path: Path,
    output_dir: Path,
    config_path: Path,
) -> dict:
    ...
    mass_low = numerical_neutrino_mass_matrix(
        result.K,
