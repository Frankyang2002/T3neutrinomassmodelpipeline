...
Policy for alpha=-1:
  * dF odd (B,C): RGBeta and the physical T3 model are both on the
    self-conjugate branch.  Validate the -16 G_Si heavy-mass insertion.
  * dF even (A,D,E): RGBeta takes a neutral/self-conjugate branch that is not
    the same field-content branch as the current Matchete T3 model.  All common
    structures are still validated, but the heavy-mass insertion is recorded
    as a branch-mismatch diagnostic rather than counted as a physics failure.
"""
...
def replace_bar_calls(expr: str) -> str:
    ...
    while "Bar[" in result:
        ...
        result = result[:start] + f"Bar_{safe}_{count}" + result[close + 1:]
        count += 1
    return result
...
def coeff(expr: sp.Expr, monomial: dict[str, int]) -> Fraction:
    out = expr
    target = {sp.Symbol(name): power for name, power in monomial.items()}
...
def checks_for(pred):
    ...
    c1 = {
        "mS1Sq*Tr_y1": ({"mS1Sq": 1, "Ty1": 1}, m1["mS1Sq*Tr_y1"]),
        "Tr_MF_y1": ({"TMy1": 1}, m1["Tr_MF_y1"]),
        "lambdaS1*mS1Sq": ({"lambdaS1": 1, "mS1Sq": 1}, m1["lambdaS1*mS1Sq"]),
        "g2_sq*mS1Sq": ({"g2": 2, "mS1Sq": 1}, m1["g2_sq*mS1Sq"]),
        "gY_sq*mS1Sq": ({"gY": 2, "mS1Sq": 1}, m1["gY_sq*mS1Sq"]),
        "lambda12*mS2Sq": ({"lambda12": 1, "mS2Sq": 1}, m1["lambda12*mS2Sq"]),
    }
    ...
    c2 = {
        "mS2Sq*Tr_y2": ({"mS2Sq": 1, "Ty2": 1}, m2["mS2Sq*Tr_y2"]),
        "Tr_MF_y2": ({"TMy2": 1}, m2["Tr_MF_y2"]),
        "lambdaS2*mS2Sq": ({"lambdaS2": 1, "mS2Sq": 1}, m2["lambdaS2*mS2Sq"]),
        "g2_sq*mS2Sq": ({"g2": 2, "mS2Sq": 1}, m2["g2_sq*mS2Sq"]),
        "gY_sq*mS2Sq": ({"gY": 2, "mS2Sq": 1}, m2["gY_sq*mS2Sq"]),
        "lambda12*mS1Sq": ({"lambda12": 1, "mS1Sq": 1}, m2["lambda12*mS1Sq"]),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-output",
        type=Path,
        default=Path("output/group_factors/scalar_mass_validation.json"),
    )
    args = parser.parse_args()

    paths = sorted(args.root.glob("T3_*_alpha_*/data/uv_rgbeta_rge.json"))
    if not paths:
        raise SystemExit(f"No RGBeta outputs found below {args.root}")

    rows = []
    branch_diagnostics = []
    overall = True

    print("T3 heavy-scalar mass group-factor validation")
    ...
    for path in paths:
        ...
        pred = scalar_mass_group_factors(d1, d2, dF, alpha)
        betas = load_betas(path)
        rgbeta_even_neutral_mismatch = (alpha == -1 and dF % 2 == 0)
        ...
        for beta_name, checks in checks_for(pred).items():
            ...
            for label, (monomial, expected_text) in checks.items():
                ...
                excluded_branch_mismatch = (
                    rgbeta_even_neutral_mismatch and label.startswith("Tr_MF_")
                )
                ...
                if excluded_branch_mismatch:
                    branch_diagnostics.append({
                        ...
                        "beta": beta_name,
                        "structure": label,
                        "physical_expected": txt(expected),
                        "rgbeta_neutral_branch": txt(got),
                        "residual": txt(residual),
                        "reason": (
                            "RGBeta uses its YF=0 neutral/self-conjugate branch "
                            "for even dF=2, while the current physical Matchete "
                            "T3 model keeps this pseudoreal representation "
                            "non-self-conjugate."
                        ),
                    })
                    details[label] = {
            ...
            print(
                f"T3-{model} {alpha:>5}  {beta_name:<6} "
                f"{validated_count:>6}  "
                f"{'PASS' if row_ok else 'FAIL'}{suffix}"
            )
    ...
    payload = {
        ...
        "physical_formulas": {
            "mSi_Tr_yi": "2 G_Si",
            "heavy_mass_yukawa_trace_vectorlike": "-4 G_Si",
            "heavy_mass_yukawa_trace_self_conjugate": "-16 G_Si",
            "self_conjugate_branch": "alpha=-1 and dF odd",
            "lambdaSi_mSi": "2(dSi+1)",
            "lambdaSiAdj_mSi_triplet": "2 C2(Si)=4",
            "g2_sq_mSi": "-6 C2(Si)",
            "gY_sq_mSi": "-6 Yi^2",
            "lambda12_other_mass": "2 d_other",
    ...
    print()
    print(
        "Even-dF alpha=-1 RGBeta heavy-mass diagnostics excluded: "
