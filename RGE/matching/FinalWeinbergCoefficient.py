"""Construct the authoritative hierarchical final Weinberg coefficient.

This module belongs to the matching boundary rather than RGE evolution.  It
combines the renormalized hard threshold contribution with the already-computed
direct running contribution, validates pole/RGE consistency and flavor
symmetry, and serializes the final coefficient used downstream.
"""
...
def normalize_pole_rge_consistency(path: Path) -> dict[str, Any]:
    """Normalize and validate the independent hard-pole/RGE diagnostic."""
    path = Path(path)
...
def _find_direct_weinberg(
    payload: Any,
    path: tuple[str, ...] = (),
) -> tuple[str, str] | None:
    """Find the direct-running Weinberg correction in a transport payload."""
...
def _contains_uv_pole(expr: str) -> bool:
    """Detect a 1/epsilon divergence in supported textual forms."""
...
def _prepare_final_weinberg_state(
    *,
    threshold_c5_path: Path,
    flavor_transport_path: Path,
    pole_rge_consistency_path: Path | None,
) -> dict[str, Any]:
...
def _physical_majorana_block_reason(state: dict[str, Any]) -> str | None:
    ...
    reasons = []
    if not state["physical_hard_ok"]:
        reasons.append(
            state["physical_hard"].get("reason")
            or "Physical Majorana hard coefficient unavailable"
        )
    if not state["direct_running_symmetric"]:
        reasons.append(
            "Direct running contribution is not manifestly symmetric under p<->q"
        )
    return "; ".join(reasons)
...
def _ordered_full_flavor_block_reason(state: dict[str, Any]) -> str | None:
    ...
    reasons = []
    if not state["structurally_full_flavor"]:
        reasons.append(
            "At least one contribution does not expose explicit p,q flavor structure"
        )
    if not state["pole_rge_validated"]:
        reasons.append(state["pole_rge_reason"])
    if not state["renormalized_hard_ok"]:
        reasons.append(
            state["renormalized_hard"].get("reason")
            or "Renormalized hard threshold was not constructed"
        )
    if not state["c12_boundary_expanded"]:
        reasons.append(
            "Direct running still contains symbolic C12[p,q] instead of the "
            "expanded y1/y2 flavor kernel"
        )
    return "; ".join(reasons)


def _build_final_weinberg_payload(state: dict[str, Any]) -> dict[str, Any]:
    threshold_info = state["threshold_info"]
    renormalized_hard = state["renormalized_hard"]
    physical_hard = state["physical_hard"]
    ...
    result: dict[str, Any] = {
        ...
        "bookkeeping": {
            "threshold_expression_already_contains_hbar": (
                state["threshold_has_explicit_hbar"]
            ),
            "direct_running_expression_already_contains_hbar": (
                state["direct_has_explicit_hbar"]
            ),
            "direct_running_requires_hbar": state["direct_requires_hbar"],
            "direct_running_object_order": "one_loop_order_one_coefficient",
        },
        "threshold_contribution_raw": {
            "source": str(state["threshold_c5_path"]),
            "expression": state["threshold_expr"],
            "contains_uv_pole": state["threshold_has_uv_pole"],
            "role": "unrenormalized Matchete hard-region diagnostic",
            "flavor_status": state["threshold_flavor_status"],
        ...
        "renormalized_hard_threshold": {
            **renormalized_hard,
            "source": str(state["threshold_c5_path"]),
            "full_flavor_symbol": state["formal_threshold"],
            "flavor_status": state["threshold_flavor_status"],
        },
        "physical_majorana_hard_threshold": {
            **physical_hard,
            ...
            "flavor_status": (
                ...
                if state["physical_hard_ok"]
                else "blocked"
            ),
        },
        "direct_running_contribution": {
            "source": str(state["flavor_transport_path"]),
            "source_location": state["direct_source"],
            "raw_expression": state["direct_expr_raw"],
            "expression": state["direct_expr"],
            "physical_majorana_expression": state["physical_direct_expr"],
            "physical_majorana_conversion_factor": 2,
            "boundary_tensor_replacements": state["boundary_replacements"],
            "flavor_status": state["direct_flavor_status"],
            "full_flavor_symbol": state["formal_direct"],
            "manifestly_symmetric_under_pq": state["direct_running_symmetric"],
        },
        "combined": {
            "full_flavor_expression": state["formal_combined"],
            "ordered_expression": state["actual_combined_ordered"],
            "physical_majorana_expression": state["actual_combined_physical"],
            "structurally_full_flavor": state["structurally_full_flavor"],
            "ready_for_ordered_full_flavor_numerics": (
                state["ready_for_ordered_full_flavor_numerics"]
            ),
            "ready_for_full_flavor_numerics": (
                state["ready_for_physical_majorana_numerics"]
            ),
            "ready_for_physical_majorana_numerics": (
                state["ready_for_physical_majorana_numerics"]
            ),
            "physical_majorana_reason": _physical_majorana_block_reason(state),
            "reason": _ordered_full_flavor_block_reason(state),
        },
        "diagnostics": {
            "threshold_has_uv_pole": state["threshold_has_uv_pole"],
            "raw_threshold_preserved": True,
            "pole_rge_consistency_source": (
                str(state["resolved_pole_rge_path"])
                if state["resolved_pole_rge_path"] is not None
                else None
            ),
            "pole_rge_consistency_validated": state["pole_rge_validated"],
            "pole_rge_consistency_reason": state["pole_rge_reason"],
            "pole_rge_consistency_payload": state["pole_rge_payload"],
            "renormalized_hard_threshold_constructed": state["renormalized_hard_ok"],
            "physical_majorana_hard_constructed": state["physical_hard_ok"],
            "direct_running_manifestly_symmetric": state["direct_running_symmetric"],
            "direct_running_requires_hbar": state["direct_requires_hbar"],
            "c12_boundary_expanded": state["c12_boundary_expanded"],
            "c12_symmetry_convention_resolved": True,
        },
        "checks": {
            "equal_scale_running_vanishes": state["equal_scale"],
            "one_generation_reduction_matches": state["one_gen_matches"],
            "log_ratio": state["log_ratio"],
        },
    }

    result["one_generation_combined_expression"] = (
        f"({state['physical_hard_expr']}) + hbar*({state['physical_direct_expr']})"
        if state["physical_hard_ok"] and state["physical_direct_expr"] is not None
        else None
    )
    return result


def _write_final_weinberg_payload(
    result: dict[str, Any],
...
def build_final_weinberg_coefficient(
    *,
    ...
    state = _prepare_final_weinberg_state(
        ...
        pole_rge_consistency_path=pole_rge_consistency_path,
    )
    result = _build_final_weinberg_payload(state)
    _write_final_weinberg_payload(result, output_path)
    return result
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "direct scalar-EFT running-generated Weinberg contribution."
        )
    )
    parser.add_argument(
        "threshold_c5",
    ...
    parser.add_argument(
        "flavor_transport",
        type=Path,
        help="Scalar-EFT full-flavor transport JSON",
    )
    parser.add_argument(
        "--output",
        type=Path,
