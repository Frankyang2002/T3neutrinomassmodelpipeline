"""Direct LLSS -> Weinberg running in the scalar-only T3 intermediate EFT.

The component calculation supplies the flavor-blind beta prefactor.  This
module restores the full lepton-flavor tensor C12[p,q] and exports the direct
one-loop contribution used at the lower scalar threshold.

Historical Wolfram symbols beginning with ``EFT1`` are preserved because they
are part of the serialized Matchete insertion contract, not the Python source
architecture.
"""
...
def _mixed_c12_boundary(flavor_seed: dict[str, Any]) -> dict[str, Any]:
    """Return the canonical mixed y1-y2 flavor boundary C12[p,q]."""
    matches = [
        term
...
def build_direct_weinberg_running(
    ...
    """Build the full-flavor direct Weinberg contribution over one interval."""
    flavor_seed = _load_success(flavor_boundary_path)
...
def _to_wolfram_scalar(raw: Any) -> str:
    """Convert a SymPy scalar to the project's Matchete coupling notation."""
...
def _c12_definition(info: dict[str, Any]) -> str:
    """Generate the full-flavor Wolfram definition of C12."""
...
def export_direct_weinberg_insertion(
    ...
    """Export the validated direct C12 -> Weinberg transport to Wolfram."""
...
# Compatibility aliases for callers and saved scripts that still use the old
# function names.
build_direct_weinberg_flavor_transport = build_direct_weinberg_running
export_direct_weinberg_matchete = export_direct_weinberg_insertion
...
def main() -> int:
    ...
    result = build_direct_weinberg_running(
