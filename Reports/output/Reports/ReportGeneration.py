...
# ---------------------------------------------------------------------------
# Shared human-facing notation.
#
# The scalar basis follows the field-contraction style used in the Ma/scotogenic
# literature wherever a direct analogue exists.  The general T3 model has two
# scalar lines and, for some doublet/triplet choices, additional independent
# contractions.  Those extra contractions are labelled by the contraction
# channel explicitly rather than by primes.
# ---------------------------------------------------------------------------
...
def paper_symbol_latex(name: str) -> str:
    """Return the common report symbol for one pipeline quantity."""
...
def paper_notation_key_lines(*, heading: str = "Notation key") -> list[str]:
    ...
    return [
        ...
        r"$y_1$ & $L\,F\,S_1+\mathrm{h.c.}$ \\",
        r"$y_2$ & $L\,F\,S_2+\mathrm{h.c.}$ \\",
        r"$M_F$ & $-\bar F\,M_F F$ \\",
        r"$T$ & $\operatorname{Tr}\!\left(Y_e^\dagger Y_e"
        r"+3Y_u^\dagger Y_u+3Y_d^\dagger Y_d\right)$ \\",
        r"$T_{\nu}^{(1)}$ & $\operatorname{Tr}\!\left(y_1^\dagger y_1\right)$ \\",
        r"$T_{\nu}^{(2)}$ & $\operatorname{Tr}\!\left(y_2^\dagger y_2\right)$ \\",
        r"$m_1^2$ & $m_1^2\,S_1^\dagger S_1$ \\",
        r"$m_2^2$ & $m_2^2\,S_2^\dagger S_2$ \\",
        r"$\lambda_1$ & $(H^\dagger H)^2$ \\",
        r"$\lambda_2^{(1)}$ & $(S_1^\dagger S_1)^2$ \\",
        r"$\lambda_2^{(2)}$ & $(S_2^\dagger S_2)^2$ \\",
        r"$\lambda_3^{(1)}$ & $(H^\dagger H)(S_1^\dagger S_1)$ \\",
        r"$\lambda_3^{(2)}$ & $(H^\dagger H)(S_2^\dagger S_2)$ \\",
        r"$\lambda_4^{(1)}$ & second independent $H$--$S_1$ quartic contraction \\",
        r"$\lambda_4^{(2)}$ & second independent $H$--$S_2$ quartic contraction \\",
        r"$\lambda_5$ & $H H S_1 S_2^\dagger+\mathrm{h.c.}$ \\",
        r"$\lambda_6$ & $(S_1^\dagger S_1)(S_2^\dagger S_2)$ \\",
        r"$\lambda_6^{(A)}$ & $(S_1^\dagger T^A S_1)(S_2^\dagger T^A S_2)$ \\",
        r"$\lambda_6^{(\times)}$ & crossed independent $S_1$--$S_2$ quartic contraction \\",
        r"$\lambda_2^{(1,A)}$ & additional independent $S_1$ self-quartic contraction \\",
        r"$\lambda_2^{(2,A)}$ & additional independent $S_2$ self-quartic contraction \\",
        r"$\alpha$ & $Y_{S_1}=\alpha/2,\quad Y_F=(\alpha+1)/2,\quad "
        r"Y_{S_2}=(\alpha+2)/2$ \\",
        r"$d_X$ & $X\sim(d_X,Y_X)$ \\",
        r"$j_X$ & $j_X=(d_X-1)/2$ \\",
        r"$C_2(d)$ & $C_2(d)=(d^2-1)/4$ \\",
        r"$T(d)$ & $T(d)=d(d^2-1)/12$ \\",
        r"$C_{LLS_1S_2}^{ij;AB}$ & $L_iL_jS_1^A S_2^B$ \\",
        r"$C_{ijab}$ & $L_iL_j\,\phi_a\phi_b$ \\",
        r"$C_5^{ij}$ & $(L_i^T C L_j)\,H H$ \\",
        r"$R_W$ & $16\pi^2\beta_{C_5}\supset "
...
def apply_paper_notation(latex: str) -> str:
    """Convert rendered implementation symbols to the common report notation."""
    if not latex:
        return latex
    ...
    rendered_map = {
        ...
        r"\lambda_{12}^{\rm Cross}": r"\lambda_6^{(\times)}",
        r"\lambda_{12}^{\rm Adj}": r"\lambda_6^{(A)}",
        r"\lambda_{H2}^{\rm Adj}": r"\lambda_4^{(2)}",
        r"\lambda_{H1}^{\rm Adj}": r"\lambda_4^{(1)}",
        r"\lambda_{S_2}^{\rm Adj}": r"\lambda_2^{(2,A)}",
        r"\lambda_{S_1}^{\rm Adj}": r"\lambda_2^{(1,A)}",
        r"\lambda_{T3}": r"\lambda_5",
        r"\lambda_{12}": r"\lambda_6",
        r"\lambda_{H2}": r"\lambda_3^{(2)}",
        r"\lambda_{H1}": r"\lambda_3^{(1)}",
        r"\lambda_{S_2}": r"\lambda_2^{(2)}",
        r"\lambda_{S_1}": r"\lambda_2^{(1)}",
    ...
    for raw, pretty in rendered_map.items():
