...
"""Consolidated T3 Yukawa-fit regression."""
...
from RGE.phenomenology.T3YukawaFit import (
    fit_three_heavy_balanced,
    reconstruct_c5,
)
...
def main() -> None:
    ...
    assert np.allclose(target.mass_matrix_ev, target.mass_matrix_ev.T, atol=1e-14)

    comparison = compare_to_nufit6_no(
        takagi_observables_real_symmetric(target.mass_matrix_ev)
    )
    assert comparison["AllFiveInside3Sigma"] is True
    assert comparison["Chi2Diagnostic"] < 1e-16

    # Non-degenerate representative loop factors in GeV^-1.
    ...
    print("PASS: target C5 is reconstructed by fitted T3 Yukawas")
    print(f"PASS: relative residual = {fit.relative_residual:.3e}")
