...
BuildClass[label_String, dims_List, alpha_Integer] := Module[
    ...
    (* In headless wolframscript RGBeta can emit harmless FrontEndObject
       messages while still returning a valid beta function.  Quiet must be
       inside Check so those messages do not create a false failure. *)
    beta1 = If[
        reg1 === "Success",
        Check[Quiet[BetaTerm[y1, 1]] // Simplify, $Failed],
        $Failed
    ];

    beta2 = If[
        reg2 === "Success",
        Check[Quiet[BetaTerm[y2, 1]] // Simplify, $Failed],
        $Failed
    ];
    ...
    If[beta1 =!= $Failed, Print["  ", beta1]];

    Print[
        "beta(y2): ",
        If[beta2 === $Failed, "Failed", "Success"]
    ];
    If[beta2 =!= $Failed, Print["  ", beta2]];
];
...
Print[""];
Print["RGBETA T3 A-E YUKAWA INVARIANT PROBE V3: COMPLETE"];
